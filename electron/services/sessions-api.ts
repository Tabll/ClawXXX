...
function writeSessionPinned(
  ...
  if (typeof keyedEntry === 'string') {
    ...
    found = true;
  } else if (keyedEntry && typeof keyedEntry === 'object' && !Array.isArray(keyedEntry)) {
...
async function loadSessionSummary(
  ...
  try {
    ...
    if (options?.metadataOnly) {
      return { sessionKey, firstUserText: null, lastTimestamp: null, workspacePath, pinned };
    }
    ...
    try {
      ...
      return { ...summarizeTranscriptMessages(sessionKey, messages, workspacePath), pinned };
    } catch {
      return { sessionKey, firstUserText: null, lastTimestamp: null, workspacePath, pinned };
    }
  } catch {
...
export function createSessionsApi(): CompleteHostServiceRegistry['sessions'] {
  return {
    ...
    summaries: async (payload) => {
      ...
      const workspaceByKey = metadataOnly
        ? new Map<string, string>()
        : await readOpenClawAcpSessionCwds(sessionKeys);
      return {
        success: true,
        summaries: await Promise.all(sessionKeys.map((sessionKey) => (
          loadSessionSummary(
            sessionKey,
            workspaceByKey.get(sessionKey.trim()) ?? null,
            { metadataOnly },
          )
        ))),
      };
