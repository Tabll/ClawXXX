...
function stripProviderPrefix(modelRef: string | undefined, providerKey: string): string | undefined {
  if (!modelRef) {
    return undefined;
  }
...
function modelCapabilitiesForModel(
  ...
  const configuredModelId = stripProviderPrefix(config.model, runtimeProviderKey);
...
async function syncCustomProviderAgentModel(
  ...
  const modelId = stripProviderPrefix(config.model, runtimeProviderKey);
...
export async function syncDefaultProviderToRuntime(
  ...
  if (!isOAuthProvider) {
    ...
    try {
      const defaultModelId = stripProviderPrefix(provider.model, targetProviderKey);
  ...
  if (
    ...
    const modelId = stripProviderPrefix(provider.model, ock);
