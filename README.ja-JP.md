...
サイドバー履歴の会話を右クリックすると、上部に固定したりインラインで名前を変更したりできます。
サイドバー履歴のグループ見出しには会話検索と、複数選択して一括削除できる「その他」メニューもあります。
...
チャットコンポーザーでは、設定済みの OpenClaw ランタイムカタログから現在の会話だけに使うモデルと思考レベルを切り替えられ、Agent に保存された既定モデルは変更されません。
...
**設定 → モデル** で、チャットモデルプロバイダー、画像生成、Embedding モデル設定をまとめて管理できます。画像生成設定では独立した OpenAI 互換の画像生成エンドポイント（Base URL、API キー、`gpt-image-2` などのモデル名）を使え、チャットは通常の Provider のまま継続できます。Embedding モデル設定は OpenClaw メモリ検索用の `agents.defaults.memorySearch` provider/model、OpenAI 互換 `/v1/embeddings` エンドポイント、ローカル GGUF オプションに加え、ソース、バッチ処理、マルチモーダル索引、ストレージ、検索ランキング、同期、キャッシュの高度な Memory Search パラメーターを書き込みます。
**設定 → Token 使用量** では、ローカル transcript に基づく Token、コスト、モデル、プロバイダー、Agent、セッション使用量をインタラクティブなチャート、セッション単位の最近使用リスト、セッションメタ情報、メッセージ/ツール統計、コスト内訳、システムプロンプトのコンテキスト円グラフを含む詳細ダイアログで確認できます。
OpenAI-compatible ゲートウェイを **Custom プロバイダー** で使う場合、**設定 → モデル → Provider 編集** でカスタム `User-Agent` を設定でき、互換性が必要なエンドポイントで有効です。
プロバイダーの編集や切り替え時、ClawX は `input: ["text", "image"]` など既存のモデル単位の能力メタデータを保持します。**設定 → モデル → プロバイダーを編集** では、設定中のチャットモデルが思考と画像入力に対応しているかを明示的に指定でき、その値は OpenClaw の provider モデル行と各 agent のモデルレジストリに書き込まれます。新しく選択した Custom プロバイダーのモデルには OpenClaw onboarding と同等の画像入力推論を適用し、不明なモデルはテキスト専用として扱います。
...
**設定 → 外観** から、アプリのアクセントカラー、全体のインターフェースフォント、macOS のフォントスムージングもカスタマイズできます。

### 🚀 自動起動設定
**設定 → 外観** から **システム起動時に自動起動** を有効化すると、ログイン後に ClawX が自動的に起動します。
...
**設定 → 詳細** で開発者モードを有効にしてから、**設定 → 開発者 → プロキシ**を開いて以下を設定します：
...
- **設定 → 開発者** では **OpenClaw Doctor** を実行でき、`openclaw doctor --json` の診断出力をアプリ内で確認できます。
