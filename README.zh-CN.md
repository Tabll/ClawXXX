...
在侧边栏历史记录中右键点击某个对话，可以将其置顶或直接进入内联重命名。
侧边栏历史分组标题还提供会话搜索，以及可进入多选批量删除的更多菜单。
当你使用 `@agent` 选择其他智能体时，ClawX 会直接切换到该智能体自己的对话上下文，而不是经过默认智能体转发。各 Agent 工作区默认彼此分离，但更强的运行时隔离仍取决于 OpenClaw 的 sandbox 配置。
聊天输入框可从已配置的 OpenClaw 运行时目录中切换当前对话使用的模型和思考级别，不会改变 Agent 保存的默认模型。
...
**设置 → Token 消耗** 会展示基于本地 transcript 汇总的 Token、费用、模型、供应商、Agent 与会话使用情况，并提供可交互图表、按会话聚合的最近使用列表，以及包含会话元信息、消息/工具统计、费用拆分和系统提示词上下文饼图的详情弹窗。
如果你通过 **自定义（Custom）Provider** 对接 OpenAI-compatible 网关，可以在 **设置 → 模型 → 编辑 Provider** 中配置自定义 `User-Agent`，以提高兼容性。
编辑或切换 Provider 时，ClawX 会保留已有的模型级能力元数据，例如 `input: ["text", "image"]`。**设置 → 模型 → 编辑 Provider** 也可以直接标记当前对话模型是否支持思考、是否支持图片输入；这些开关会写入 OpenClaw 的 provider 模型行和每个 agent 的模型注册表。新选择的自定义 Provider 模型会使用与 OpenClaw onboarding 一致的图片输入能力推断；未知模型默认按纯文本模型处理。
如果兼容网关的 `/models` 因非鉴权原因不可用，ClawX 会在校验 API Key 时自动降级为轻量的 `/chat/completions` 或 `/responses` 探测。

### 🌙 自适应主题
支持浅色模式、深色模式或跟随系统主题。ClawX 自动适应你的偏好设置。
你也可以在 **设置 → 外观** 中自定义应用强调色、全局界面字体，并在 macOS 上开启字体平滑。
