...
describe('DeepSeek Harness canonical Skill runtime overlay', () => {
  it('scans only its managed Skill root and never follows Skill symlinks', () => {
    ...
    expect(runtimeHost).toContain('includeDefaultRoots: false');
    expect(runtimeHost).toContain("customSkillDirs: [join(config.dataDir, 'skills')]");
    expect(runtimeHost).toContain('watchFollowSymlinks: false');
