...
function cleanupWhatsAppLoginCredentials(accountId: string): void {
  const authDir = join(testHome, '.clawx', 'kernel-config', 'openclaw', 'credentials', 'whatsapp', accountId);
  if (existsSync(authDir)) {
    rmSync(authDir, { recursive: true, force: true });
    const parentDir = join(testHome, '.clawx', 'kernel-config', 'openclaw', 'credentials', 'whatsapp');
...
describe('WhatsApp login cancel cleanup logic', () => {
  const whatsappCredsDir = () => join(testHome, '.clawx', 'kernel-config', 'openclaw', 'credentials', 'whatsapp');
...
describe('listConfiguredChannels WhatsApp detection', () => {
  ...
  it('reports WhatsApp as configured when credentials directory has a session', async () => {
    const authDir = join(testHome, '.clawx', 'kernel-config', 'openclaw', 'credentials', 'whatsapp', 'default');
    mkdirSync(authDir, { recursive: true });
    writeFileSync(join(authDir, 'creds.json'), '{}', 'utf8');
  ...
  it('does NOT report WhatsApp as configured when credentials directory is empty', async () => {
    ...
    const whatsappDir = join(testHome, '.clawx', 'kernel-config', 'openclaw', 'credentials', 'whatsapp');
  ...
  it('does NOT report WhatsApp as configured when credentials directory does not exist', async () => {
    ...
    mkdirSync(join(testHome, '.clawx', 'kernel-config', 'openclaw'), { recursive: true });
