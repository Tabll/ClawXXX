...
test.describe('OpenClaw Dreams', () => {
  ...
  test('renders the native Dreams page and runs a maintenance action', async ({ electronApp, page }) => {
    ...
    await expect(page.getByTestId('dreams-tab-scene')).toBeVisible();
    await expect(page.getByTestId('dreams-mascot')).toBeVisible();
    await expect(page.getByTestId('dreams-phrase')).toBeVisible();

    await page.getByTestId('dreams-tab-diary').click();
    await expect(page.getByText('Native dreams page landed')).toBeVisible();
    await page.getByTestId('dreams-diary-entry-1').click();
    await expect(page.getByText('Older note')).toBeVisible();

    await page.getByTestId('dreams-tab-advanced').click();
    await expect(page.getByText(/Signals and artifacts|信号与产物/)).toBeVisible();
