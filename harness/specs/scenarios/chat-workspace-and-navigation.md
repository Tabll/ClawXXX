...
This scenario covers inheriting the selected Conversation's canonical workspace when creating a new Chat; selecting persisted recent, known-Conversation, or newly browsed workspaces while the local Conversation remains unbound; validating availability before router admission; deriving the canonical SQLite title atomically from the first raw prompt; ignoring runtime fallback labels and transcript metadata; recovering from deleted workspace paths; marking unavailable non-default groups; hard-deleting canonical Conversations after confirmation; capturing workspace in the immutable run/Agent snapshot; targeting another Agent without losing workspace or first prompt; restoring historical canonical context; renaming display-only workspace labels; navigating grouped Conversations with busy, unread, kernel provenance, and relative-time status; browsing the workspace; previewing authorized local HTML and Office documents; and jumping among user questions without changing conversation width.
