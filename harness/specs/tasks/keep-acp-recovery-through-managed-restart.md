...
intent: Keep an accepted ACP prompt recoverable for a policy-derived recovery window, and avoid renderer-only reload races.
touchedAreas:
...
acceptance:
  ...
  - The recovery budget keeps the unacknowledged-send deadline unchanged and documents how its current inputs derive the bounded recovery window.
