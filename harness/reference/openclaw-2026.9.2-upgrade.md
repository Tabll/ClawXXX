...
更新：2026-09-07。仓库开发依赖、补丁、源码描述符和 CI 运行时定义已切换为 **2026.9.2+clawx.10**。+9 修复 Windows 物理路径别名与插件安装所有者关联；+10 修复清单缓存仍保留首个短路径、导致后续摘要读取失败的问题。CI 公证改为保存提交 ID 后有界重试状态查询，详见 [Windows CI 修复](windows-runtime-ci-repair.md)。这不是已发布声明：用户已安装的不可变运行时和在线 catalog 未被修改，签名、公证、五平台 clean-machine 与发布演练仍须通过。
...
| runtime | 2026.9.2+clawx.10，patch revision 10 |
