...
`2026.9.2+clawx.10`. Canonical typed history hydrates a new incognito session per
...
- The npm patch-base and independent Node downloaders create private `.clawx-*-` staging directories in the destination's parent, keeping download, extraction and final rename on one filesystem even when Windows `%TEMP%` is on C: and the checkout is on D:. Existing destinations are rejected; integrity, archive and identity checks precede publication, and `finally` removes owned staging on success or failure. There is no cross-volume copy fallback. Offline CLI fixtures reproduce `EXDEV` on the old implementation and exercise both success and fail-closed cleanup paths before expensive build/signing work.
- The Lark published CommonJS entry is loaded by a fresh asynchronous native Node child, with the actual `register` export asserted, a 60-second hard child timeout and a 75-second dedicated test deadline. Both repaired `import.meta` sites are separately parsed as CommonJS. The rest of Vitest retains its default timeout; no dependency mocks, forced successful process exit, retries or skipped imports mask compatibility or process-lifetime failures. Real pre-seal/extracted-artifact Gateway, ACP and Channel probes remain mandatory.
