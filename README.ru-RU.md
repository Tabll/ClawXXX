...
Кандидат DSH — `0.1.5-rc.2+clawx.14` (предварительный выпуск). Адаптированы системные сообщения V3, persona отдельного Run и исходные append-события; замена контекста модели не дублирует ответы и расход токенов. Общая SQLite остаётся единственным хранилищем. Установленные ядра обновляются только после проверки и публикации CI-артефактов. См. [текущий контракт](harness/reference/kernel-upgrade-2026-09-12.md).

Исходники и зависимости OpenClaw обновлены до `2026.9.4+clawx.14`, включая Discord/WhatsApp. Перебазированные `.mjs`-патчи сохраняют ограниченный журнал ACP в памяти, общую историю, настройки под управлением Main и строгий допуск Channels. Node остаётся 24.20.0. Локальные результаты и ожидающие проверки платформ указаны в [плане обновления](harness/reference/kernel-upgrade-2026-09-12.md).

Оба ядра +clawx.13 с Node 24.20.0 прошли все 25 staging-задач для пяти целевых платформ, четыре нотариальные проверки macOS (Accepted) и Electron E2E для того же исходного кода на трёх платформах; см. [результаты CI](harness/reference/windows-runtime-ci-repair.md). Windows использует только подписи артефактов, без Authenticode. Статус production-каталога, восстановление защищённой публикации и проверка раздачи отражены в [журнале публикации](harness/reference/kernel-automatic-release.md); проверки реальных аккаунтов ещё не завершены.

Успех полной сборки ядер и E2E того же SHA автоматически запрашивает защищённую публикацию. Версионные пакеты в COS/GitHub неизменяемы; после проверки всех платформ на обоих зеркалах заменяется только подписанный каталог актуальных пакетов. Старые пакеты удаляются после истечения всех ссылающихся каталогов плюс 24 часа; подписанные журналы сохраняются, прерванные операции возобновляются. Ежедневное обслуживание обновляет обычно семидневный каталог в пределах срока ключей/артефактов, по-прежнему требуя одобрения. Начальная bootstrap-публикация и проверка реальной раздачи остаются отдельными этапами. См. [дизайн публикации](harness/reference/kernel-automatic-release.md).

После замены каталога ограниченные повторы ожидают точного подписанного выпуска, затем выполняется строгая повторная проверка перед удалением. Старый или временно отсутствующий каталог не считается успехом; конфликт подписанных каталогов немедленно останавливает публикацию.

Метка Pre-release у фиксированной страницы ресурсов ядер GitHub служит только для исключения из `latest` основного приложения. Подписанный каталог ядер остаётся `production`; версии, подписи и постоянные URL загрузки не меняются.

- **Модель процессов**: Electron Main управляет системной интеграцией, единственным DataService, Package Manager и отдельным Supervisor каждого kernel. OpenClaw и DSH работают параллельно; Renderer/runtime не открывают каноническую ClawX SQLite напрямую и не соединяются друг с другом.
- **Проверка runtime**: установка и повторное сканирование проверяют файлы не более чем в 8 параллельных операциях, сохраняя все проверки подписанных хешей, размеров и путей. Каждая распаковка использует отдельный кеш каталогов на 256 записей для ограничения накладных расходов больших пакетов; вытеснение лишь вызывает повторную проверку файловой системы, не ослабляя защиту путей. Зафиксированный патч инструментов хоста заменяет отображение небольших файлов в память обычной записью в Windows, сохраняя резервирование путей tar и все проверки. Установленные файлы доступны только для чтения; ошибка установки этой защиты блокирует установку. В Windows атомарное перемещение каталога повторяется только при временных `EPERM`/`EBUSY` с суммарным ожиданием не более 1,5 секунды; постоянная блокировка остаётся ошибкой без копирования или ослабления прав. Clean-machine CI записывает длительность распаковки, проверки хешей и установки защиты от записи, включая ошибки и тайм-ауты; необязательная диагностика не влияет на результат проверки.
...
При первом запуске `pnpm dev` подготавливает отдельный Node 24.20.0 для ядер. Gateway, ACP, CLI и Doctor больше не запускаются через Electron Helper; установленные ядра используют проверенный Node из своего пакета. Перед прямым запуском Electron выполните `pnpm run kernel:dev:prepare`. Для локальной загрузки нужны независимо проверенные открытые production-ключи; см. [настройку Node и доверия](harness/reference/electron-kernel-node-launch.md).

Авторизация Agent записывается через SQLite SDK выбранного ядра: ClawX не создаёт неполную нативную схему и не перезаписывает её версию. Перед исправлением существующих данных нужны резервная копия и проверка на изолированной копии; см. [владение схемой и восстановление](harness/reference/openclaw-agent-schema-ownership.md).

В режиме разработки и в готовом приложении установленные пакеты ядер имеют приоритет над каталогами разработки. Остановленный DeepSeek Harness доступен для запуска сразу после установки или исправления. Активация OpenClaw требует полного перезапуска ClawX для пересоздания связей ACP/Channels. Настройки различают перезапуск приложения и загруженную, но не активированную версию (остановите ядро и нажмите «Обновить»). См. [выбор и активацию сред](harness/reference/installed-kernel-activation.md).

### Требования

- **Node.js**: 22.22.3+, 24.15.0+ или 25.9.0+ в пределах соответствующей основной версии. Рекомендуется Node 24.20.0 LTS; загружаемые ядра и runtime CI фиксируют эту версию (ABI модулей 137). На Windows избегайте Node 24.0–24.15 из-за upstream-сбоя TCP без диагностики. См. [результаты исправления](harness/reference/windows-runtime-ci-repair.md).
...
pnpm run build:vite  # Генерация bridge расширений и сборка общих UI/Electron bundle
