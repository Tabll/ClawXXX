...
- **Проверка runtime**: установка и повторное сканирование проверяют файлы не более чем в 8 параллельных операциях, сохраняя все проверки подписанных хешей, размеров и путей. Каждая распаковка использует отдельный кеш каталогов на 256 записей для ограничения накладных расходов больших пакетов; вытеснение лишь вызывает повторную проверку файловой системы, не ослабляя защиту путей. Установленные файлы доступны только для чтения; ошибка установки этой защиты блокирует установку. В Windows атомарное перемещение каталога повторяется только при временных `EPERM`/`EBUSY` с суммарным ожиданием не более 1,5 секунды; постоянная блокировка остаётся ошибкой без копирования или ослабления прав. Clean-machine CI записывает длительность этапов, включая ошибки и тайм-ауты.
