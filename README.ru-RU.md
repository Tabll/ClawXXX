...
Текущие исходники DSH адаптированы к `0.1.3-alpha.1+clawx.11`: потоковые события v2 используют прежнее общее хранилище SQLite. Это alpha, upstream сообщает о снижении производительности. Обновление установленного ядра требует проверки и публикации нового CI-артефакта. См. [контракт совместимости](harness/reference/deepseek-harness-0.1.3-upgrade.md).

Исходники и зависимости разработки OpenClaw обновлены до `2026.9.2+clawx.11`. Рабочий bridge создаёт отдельную сессию в памяти для каждого Run из общей SQLite-истории; адаптированы настройки Agents, моделей, прав и 7 плагинов Channels. Изолированные проверки настоящих Gateway/ACP и собранного payload покрывают инструменты, отмену, восстановление после сбоя и отказ во входящем сообщении без записи native-истории. Установленным ядрам нужен новый проверенный CI-артефакт. Подписание и публикация для 5 платформ, а также проверки реальных аккаунтов ещё не выполнены. См. [дизайн и результаты](harness/reference/openclaw-2026.9.2-upgrade.md).
