...
- **ACP Chat**: Chat UI взаимодействует с OpenClaw через [ACP (Agent Client Protocol)](https://agentclientprotocol.com), обеспечивая относительно стабильную поверхность чат-протокола поверх часто итерируемого OpenClaw. ACP работает через stdio bridge под управлением Main и поддерживает аутентифицированное воспроизведение истории после перезагрузки конфигурации, потоковую выдачу при навигации, объявленные сессией настройки модели/рассуждений, использование контекста и ручное сжатие, последовательную очередь последующих сообщений, а также медиа, вложения и файловые операции, проверенные Main. Transcript используется только для ограниченного дополнения метаданных при наведении и отсутствующих в ACP ресурсов, а не как второй источник истории. Если защищённый перезапуск Gateway прерывает уже принятый ход, исправленная среда OpenClaw явно связывает восстановительный run с исходным ACP prompt, чтобы последующий текст и активность инструментов продолжались в том же ходе в памяти; последующее воспроизведение истории восстанавливает сохранённые границы инструментов как нативные обновления ACP.
