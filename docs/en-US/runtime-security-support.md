...
- The local package manager retains the active and previous verified compatible runtime for rollback; this is not permanent cloud retention. Production catalogs offer one latest version per kernel/target. Old packages are removed only after the maximum expiry of all referencing catalogs plus 24 hours, using a signed exact inventory. Installed bytes and canonical data are untouched; cloud reinstall of that old version becomes unavailable after cleanup.
...
Complete trusted runtime acceptance and same-source three-platform E2E automatically queue the protected production environment. Immutable signed release records and versioned package bytes precede the catalog switch, with all-target two-mirror Range/If-Range, strong ETag and signed-size checks before and after switching. Daily protected maintenance renews normally seven-day catalogs with less than 48 hours remaining, never beyond descriptor/key validity or without required approval. Cleanup preserves journals/receipts and rejects Enabled/Suspended COS versioning. See the [automatic publication design](../../harness/reference/kernel-automatic-release.md).
