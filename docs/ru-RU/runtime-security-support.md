...
Предыдущая версия сохраняется локально, а не навсегда в облаке. Production catalog предлагает одну последнюю версию на kernel/target. Старые пакеты удаляются по точному подписанному списку после максимального срока всех ссылающихся каталогов плюс 24 часа; затем повторная загрузка старой версии из облака недоступна. Полная runtime/E2E-проверка одного SHA автоматически запрашивает защищённую публикацию с проверками Range/If-Range, сильного ETag и размера всех платформ на обоих зеркалах до и после смены каталога. Ежедневное обслуживание продлевает обычно семидневный каталог при остатке менее 48 часов только в пределах срока artifact/key и с прежним одобрением. Подписанные journals/receipts сохраняются, Enabled/Suspended COS versioning запрещён. См. [дизайн публикации](../../harness/reference/kernel-automatic-release.md).
