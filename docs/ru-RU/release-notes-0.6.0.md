...
Адаптированы несовместимые изменения DSH 0.1.3 в композиции сервисов, завершении потоков и SessionHandle. Текст неудачной попытки заменяется, Usage учитывается один раз на запрос; общая история не меняется. Upstream сообщает о снижении производительности. Обновление исходников `.11` не означает завершение подписи, нотарификации или публикации. [Совместимость](../../harness/reference/deepseek-harness-0.1.3-upgrade.md).
...
- DeepSeek Harness `0.1.3-alpha.1+clawx.11` добавлен вторым optional kernel; оба могут работать одновременно и независимо обновляться, завершаться с ошибкой и откатываться.
