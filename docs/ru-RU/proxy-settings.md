...
- Сохранение прокси немедленно применяет networking Electron. Только если optional OpenClaw установлен и изменился launch environment, перезапускается его owned Gateway; DeepSeek Harness и будущие kernels не перезапускаются, отсутствующий runtime не запускается.
