...
Base installer не содержит AI kernel. При первом использовании можно установить OpenClaw, DeepSeek Harness, оба или ни одного. Platform-specific signed CI artifacts поддерживают progress, cancel/resume, repair, независимые update/rollback и diagnostics. Оба kernel могут быть ready и streaming одновременно; ошибка или update одного не останавливают другой.

Они используют одинаковые Chat, Providers/Models, Agents, Channels, Cron, Skills, Usage, Diagnostics и canonical contracts. Операция Both сохраняет partial success. Conversation не принадлежит kernel: для следующего turn можно выбрать другой, UI показывает provenance, а ClawX передаёт только portable/redacted history. DSH Web UI не встраивается.

Новая history хранится один раз в ClawX SQLite/Blob. После uninstall kernel её можно искать, переименовывать, экспортировать и удалять; Cron/Channel используют те же Conversation/Run. Старые upstream history намеренно не мигрируются и не служат fallback. См. [data policy](data-security-retention.md).
...
Выбор расписания разделён на вкладки **Повтор** и **Однократно**. Повтор предлагает частоты «Ежечасно», «Ежедневно», «По будням», «Еженедельно» и «Свой» (произвольный cron) со встроенным выбором времени и дня недели. Однократно запускает задачу один раз в выбранные дату и время с отображением дня недели. Одноразовая задача планируется только в будущем, а её lifecycle завершает Main-owned ClawXScheduler после commit canonical completion.
...
ClawX поставляет document-processing skills (`pdf`, `xlsx`, `docx`, `pptx`) как host skill packages. Canonical Skills catalog владеет desired state и проецирует независимые physical copies в каждый compatible installed kernel. Managed copy OpenClaw создаётся только при activation OpenClaw; запуск base app без kernel не пишет в OpenClaw directory.

Одна страница Skills показывает canonical packages, workspace sources и per-kernel projection state. OpenClaw-specific sources сканируются только при наличии optional runtime. Его artifact оставляет только allowlisted bundled `skill-creator`; прочие upstream bundled skills удаляются при build optional artifact, а stale managed projections reconciled без изменения DSH copies.
