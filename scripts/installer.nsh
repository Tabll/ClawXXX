...
;
; Install: enables long paths. Optional-kernel CLI wrappers are generated by
; ClawX only after their verified runtime has been activated.
...
!macro customCheckAppRunning
  ...
  ${if} $R0 == 0
    ${if} ${isUpdated}
      ...
      ${nsProcess::FindProcess} "${APP_EXECUTABLE_FILENAME}" $R0
      ${if} $R0 != 0
        # App exited cleanly. ClawX-owned kernel process groups are stopped by
        # the app lifecycle; never kill a generic runtime executable by name.
        Goto done_killing
      ${endIf}
    ...
    # This covers ClawX.exe (multiple Electron processes) and any child
  ...
  ; Kill only the ClawX application executable by name. Optional kernels may be
  ; shared with other products, so global kernel executable kills are forbidden.
  nsExec::ExecToStack 'taskkill /F /T /IM "${APP_EXECUTABLE_FILENAME}"'
  Pop $0
  Pop $1
  ...
  _found_free_stale:
    ...
    IfErrors 0 _stale_moved
      ...
      ; Antivirus/file locks can make rd /s /q take several minutes.
