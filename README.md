...
Runtime and same-source E2E success now automatically queue protected production publication. Versioned packages remain immutable on Tencent COS and GitHub; only the signed latest-per-target catalog is replaced after every target passes both-mirror verification. Old packages are retired after all referencing catalogs expire plus 24 hours, with signed audit records and resumable cleanup. Daily protected maintenance renews seven-day catalogs within artifact/key validity; reviewer approval is still required. Initial bootstrap and live acceptance remain separate from local implementation tests. See the [publication design](harness/reference/kernel-automatic-release.md) and [runbook](docs/zh-CN/operations/kernel-runtime-release-runbook.md).

After catalog replacement, bounded propagation checks must reach the exact signed release before final strict readback or cleanup. Stale/missing mirrors never count as success, and signed catalog conflicts stop publication.

The fixed GitHub kernel resource page carries a Pre-release label only to exclude it from host-app `latest` discovery. The signed kernel catalog remains `production`; versions, signatures and fixed download URLs are unaffected.
...
- **Runtime verification**: Installation and rescans use bounded eight-worker file verification, retaining every signed hash, size and path check. Extraction uses an independent 256-entry directory cache to bound large-package overhead; eviction triggers filesystem rechecks without weakening path protection. A pinned host-tooling patch uses ordinary Windows file writes instead of memory-mapped small-file writes, retaining tar's path reservations and all checks. Installed files remain read-only; failure to apply that protection rejects installation. Windows atomic directory moves allow at most 1.5 seconds of retry delays for transient `EPERM`/`EBUSY` locks; persistent locks still fail without copying or weakening permissions. Clean-machine CI records extraction, hashing and read-only sealing phase timings, including failures and timeouts; optional diagnostics cannot change verification results.
...
- **Node.js**: 22.22.3+, 24.15.0+, or 25.9.0+ within the corresponding supported major line. Node 24.20.0 LTS is recommended and pinned in downloadable runtimes and runtime CI (module ABI 137). On Windows, avoid Node 24.0–24.15 because of the upstream silent TCP-connect crash; see the [repair evidence](harness/reference/windows-runtime-ci-repair.md).
