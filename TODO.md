...
- [x] `MK-1928` +9 已经由 `1b459ff7` 提交/推送并 dispatch/审批 [34048363051](https://github.com/Tabll/ClawXXX/actions/runs/34048363051)：8/10 build 成功；OpenClaw Windows 在真实 Gateway 前的清单失效探针失败，DSH Intel macOS 已签名但公证查询网络超时。三平台 Electron E2E 通过，单/双内核 clean-machine 跳过；不是全绿或生产发布。
- [x] `MK-1929` 真实 Windows 长/8.3 短 TEMP 对比稳定复现：短路径清单被缓存、root 后续 canonical 化，`path.relative` 生成逃逸路径导致安全读取失败，manifestHash 为空；修改清单仍得到 persisted。长路径通过。不是插件副本选错，也不是凭据或等待时限问题。
...
- [x] `MK-1940` +13 修复提交 `45bb92609a2349ed89e50c45c64cac53f1aa2620` 已推送；[完整 staging #21](https://github.com/Tabll/ClawXXX/actions/runs/34205494108) 正常 dispatch/审批后首次 attempt 的 25/25 任务全部成功：双内核五目标构建、10 单内核与 5 双内核 clean-machine，无失败/取消/跳过任务。四份 macOS 签名/Accepted 公证报告及归档摘要已核验，共 35 份制品/报告；[同 SHA E2E #35](https://github.com/Tabll/ClawXXX/actions/runs/34195995688) 三平台全通过。COS/catalog/生产发布仍不在本轮范围，Windows Authenticode 继续暂缓。
...
- [x] `MK-1966` 读取 [第 17 轮 34182369085](https://github.com/Tabll/ClawXXX/actions/runs/34182369085) 最终 25 项任务及两个 Windows 失败日志：双内核首次安装 628468 ms、634333 ms 开始修复、900000 ms 超时；OpenClaw 单内核 571913 ms 安装完成，重扫超出 600000 ms。两种 Windows 构建均通过 137 项路径等前置；不是此前 POSIX-only 夹具复发。[同 SHA E2E #31](https://github.com/Tabll/ClawXXX/actions/runs/34182316212) 三平台成功，保留 35 份 runtime/build/evidence artifacts，不把 23/25 记为全绿。
