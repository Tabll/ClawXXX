...
- **Token usage history implementation**: Dashboard token usage is read only from canonical SQLite `usage_entries`. OpenClaw provider responses and DeepSeek Harness SessionEvents are normalized during live runs, keyed by stable `(run_id, event_key)` identities, and persisted once; provider retries, replayed delivery, and terminal fallback cannot double-charge. Missing token or cost fields remain absent/unknown rather than becoming zero. Do not reintroduce transcript, runtime-directory, console-log, Gateway-history, or native usage-file scans. Deleting a canonical Conversation removes its related run/usage rows through the same SQLite authority.
